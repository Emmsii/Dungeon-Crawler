...
public class Noise {
	...
	public int[][] startNoise(int w, int h, long seed, double featureSize, double percistence, int octave, double amplitude){
		int result[][] = new int[w][h];
		
		for(int y = 0; y < h; y++){
			for(int x = 0; x < w; x++){
				double c = pNoise((double) x * featureSize - w, (double) y * featureSize - h, seed, percistence, octave, amplitude);
				c *= 128.0;
				c += 127.0;
				if(c > 255.0) c = 255.0;
				if(c < 0.0) c = 0.0;
